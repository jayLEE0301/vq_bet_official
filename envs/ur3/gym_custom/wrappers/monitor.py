...
class Monitor(Wrapper):
    ...
    def _start(self, directory, video_callable=None, force=False, resume=False,
        ...
        if self.env.spec is None:
            logger.warn("Trying to monitor an environment which has no 'spec' set. This usually means you did not create it via 'gym_custom.make', and is recommended only for advanced users.")
            env_id = '(unknown)'
        else:
 ...
 You should use a unique directory for each training run, or use 'force=True' to automatically clear previous monitor files.'''.format(directory, ', '.join(training_manifests[:5])))
    ...
    def close(self):
        ...
        logger.info('''Finished writing results. You can upload them to the scoreboard via gym_custom.upload(%r)''', self.directory)
